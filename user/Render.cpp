...
void Visuals::WWE() {
	...
	for (int i = 0; i < PlayerDataXD->Count; ++i) {
		...
		app::Transform* TransformObj = app::GameObject_get_transform(goHeadObj, nullptr);
		if (!TransformObj) continue;

		app::Vector3 EnemyHeadLocation = app::Transform_get_position(TransformObj, nullptr);
